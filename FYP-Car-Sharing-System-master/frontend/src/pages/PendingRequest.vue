<template>
  <div class="background">
    <div class="container" border="3px" style="padding:20px">
        <md-dialog-confirm
      :md-active.sync="confirmApprove"
      md-title="Confirm to Approve this transaction?"
      md-content=""
      md-confirm-text="Confirm"
      md-cancel-text="Cancel"
      @md-cancel="confirmApprove = false"
      @md-confirm="approve()" />

      <md-dialog-confirm
      :md-active.sync="confirmDecline"
      md-title="Confirm to Decline this transaction?"
      md-content=""
      md-confirm-text="Decline"
      md-cancel-text="Cancel"
      @md-cancel="confirmApprove = false"
      @md-confirm="decline()" />
                ...
                <table style="width:100%" class="resultTable">
                    ...
                    <tr>
                        <td colspan="4" class="clsValue">
                            ...
                            <b-button icon-left="account" type="is-light" @click.prevent="chat(item)"
                                >Chat
                            </b-button> 
                                &nbsp;
                            <b-button type="is-danger" @click.prevent="confirmDeclines(item.car.id, item.id)"
                            >Decline</b-button
                            >
                            <b-button type="is-success" @click.prevent="confirmApproves(item.car.id, item.id)"
                                >Approve</b-button
                                >
                            </md-card-actions>
                            </div>
...
export default {
  ...
  async created() {
    ...
    const result = await rent.pendingRequest(this.rentObj);
  ...
  methods: {
    chat(item) {
      console.log(item.id)
        this.$router.push({ path: `/hostchat/${item.id}` });
    },
    confirmApproves(carId, rentId){
        this.carId = carId;
        this.confirmApprove = true;
        this.rentId = rentId;
    },
    confirmDeclines(carId, rentId){
        this.carId = carId;
    ...
    async approve(){
        ...
        this.rentObj.paramIn_1 = localStorage.userId;

        try{
            const result = await rent.approveRental(this.rentObj);
            location.reload();
        } catch(err){
            console.log(err)
            alert(err)
    ...
    async decline(){
        ...
        try{
            const result = await rent.declineRental(this.rentObj);
            location.reload();
        } catch(err){
            console.log(err)
            alert(err)
