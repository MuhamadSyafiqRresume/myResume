<template>
  <div class="table">
    ...
    <b-tooltip style="float:right;" label="Reload Data">
      <md-button class="md-icon-button md-dense md-raised md-primary" @click="allCar()">
    ...
    >
      <template slot-scope="props">
        <b-table-column field="carModel" label="Car Model" searchable>
          {{ props.row.carModel }}
        </b-table-column>

        <b-table-column field="username" label="Car Owner" searchable>
          {{ props.row.user.username }}
        </b-table-column>

        <b-table-column field="registrationNumber" label="Registration Number" searchable>
        ...
        <b-table-column field="status" label="Status" centered sortable>
          <span v-if="props.row.isActive == 'true'" style="font-size: 2em; color: #84f092;">
        ...
        <b-table-column field="availableFrom" label="Available From" centered sortable>
          <span v-if="props.row.availableFrom != null" class="tag is-success">
        ...
        <b-table-column field="availableTo" label="Available To" centered sortable>
          <span v-if="props.row.availableFrom != null" class="tag is-success">
  ...
  methods: {
    async allCar() {
      try {
        ...
        if (!clog.err) {
          const result = await car.allCar(this.carObj);
          console.log(result);
          for (var i = 0; i < result.length; i++) {
            result[i].isActive = result[i].isActive.toString();
          }
          this.data = result;
        } else {
