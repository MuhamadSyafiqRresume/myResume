<template>
  <div class="background">
    <div class="container" border="3px" style="padding:20px">
      <md-dialog :md-active.sync="rateDialog">
        <md-dialog-title>Rate Your Experience</md-dialog-title>
        <md-dialog-content>
            <Rating :grade="3" :maxStars="5" :hasCounter="true" v-on:giveStar="updateStar($event)" />       
        </md-dialog-content>
        <md-dialog-actions style="margin-left:auto; margin-right: auto;">
            <md-button class="md-danger" @click="rateDialog = false"
              >Close</md-button
            >
            <md-button class="md-success" @click="rates()"
              >Rate</md-button
            >
          </md-dialog-actions>
      </md-dialog>
                ...
                <table style="width:100%" class="resultTable">
                    ...
                    <tr>
                        <td rowspan="5" style="width:100px;border:none;">
                            <figure  >
                                <img :src="item.car.carImage" class="cardImage"/>
                            </figure>
                        </td>
                    ...
                    <tr>
                        <td colspan="4" class="clsValue">
                            ...
                            <b-button v-if="item.rate == null" type="is-success" @click="rateClicked(item.id)"
                                >Rate</b-button
                                >
                                <showRating v-else :grade="item.rate" :maxStars="5" :hasCounter="false"  />
                            </md-card-actions>
                            </div>
                        </td>
                    </tr>
...
export default {
  ...
  async created() {
    ...
    // const result = await rent.pendingReservation(this.rentObj);
    const result = await rent.completedReservation(this.rentObj);
  ...
  methods: {
    ...
    async rates() {
      ...
      this.rentObj.rate  = this.rate;
      const result = await rent.rate(this.rentObj);
